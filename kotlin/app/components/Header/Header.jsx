...
export default function Header() {
    ...
    return (
        <header className="relative flex items-center h-16 px-8 bg-[#27282c] border-b border-white/20 z-[906]">
            <div className="flex flex-grow items-center">
                <a href="https://kotlinlang.org" className="block h-6 w-[100px] cursor-pointer flex items-center space-between gap-2">
                    <svg className="flex items-center" height="24" viewBox="0 0 100 24" xmlns="http://www.w3.org/2000/svg">
                        <radialGradient id="kotlin-logo-large_svg__a" cx="22.432" cy="3.493" r="21.679" gradientTransform="translate(-.19 .042) scale(.9998)" gradientUnits="userSpaceOnUse"><stop offset="0.003" stop-color="#e44857"></stop><stop offset="0.469" stop-color="#c711e1"></stop><stop offset="1" stop-color="#7f52ff"></stop></radialGradient>
                ...
                <a class="text-[13px]/4 self-start text-white/75 hover:text-white/100" href="https://github.com/JetBrains/kotlin/releases/tag/v1.6.20" target="_blank">v1.6.20</a>
            </div>

            <nav className="flex items-center gap-8 h-16 gap-[32px]">
                {navData.main.content.map((item) => (
                    <a
                        key={item.title}
                        href={item.url}
                        className={`text-white/75 hover:text-white border-b border-transparent border-t hover:border-b-white/40 transition-colors/0 h-16 flex items-center text-base`}
                    >
                        {item.title}
                    </a>
                ))}
            </nav>
            <button className="h-6 w-6 ml-8"></button>
        </header>
    );
}